...
cdef class MatrixFactorization_Cython_Epoch:
    ...
    def _init_minibatch_data_structures(self):
        ...
        self.mini_batch_sampled_items = np.zeros(self.batch_size*2, dtype=np.int32)
        self.mini_batch_sampled_users = np.zeros(self.batch_size, dtype=np.int32)

        self.mini_batch_sampled_items_flag = np.zeros(self.n_items, dtype=np.int32)
        self.mini_batch_sampled_users_flag = np.zeros(self.n_users, dtype=np.int32)
